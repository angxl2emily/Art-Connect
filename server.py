        ...
        if params_dict["medium"] == "-":
            if params_dict["price"] == "-":
                if params_dict["inStock"] == "-":
                    update_query = "DELETE FROM poffers_products P WHERE P.user_id = :user_id AND P.product_id = :product_id"

        else:
            # if no product_id, but other fields => add to table
            if params_dict["product_id"] == "-":
                max_cursor = g.conn.execute(
                    text("""SELECT MAX(product_id) FROM poffers_products""")
                )
                max_id = max_cursor.fetchone()[0]
                ...
                cursor = g.conn.execute(
                    text(
                        """INSERT INTO poffers_products(product_id, user_id) VALUES ("""
                        + str(params_dict["product_id"])
                        + ","
                        + ":user_id)"
                    ),
                    params_dict,
